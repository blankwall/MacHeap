...
const int Quantum = 0x200;
const int Minimum = 1010;
...
main()
{
    ...
    region_spray(Minimum, 0x40);
    asm("int3\n\t");

    printf("allocating 0x%x slots\n", 0x20 * Minimum/Quantum);
    for (i=0; i < 0x20; i++)
        array[i] = malloc(Minimum+Quantum);
    ...
    printf("allocating 2 objects (4 chunks)\n");
    for (i = 0; i < 2; i++) {
        printf("%d:%p\n", i, malloc(Minimum));
    }
    asm("int3\n\t");
